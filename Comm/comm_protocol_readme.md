...
> 这两个字段就能直接用上，**协议不需要改动**。
>
> 调参时想给一个**保持住**的速度给定，用 CLI 的 `spd` 命令，它走的是 `pid.c` 里
> 专门的手动目标通道（见"七、VOFA+ 波形调参"里的"用滑块设速度目标"），
> 不受这里的斜坡模型和 350ms 指令看门狗影响。开机默认就在 manual；要用蓝牙前进/
> 后退，先发 `spd off` 交还遥控。
...
| `spd` | 设定速度环目标做阶跃，见"七、VOFA+ 波形调参" |
| `vofa` | 开关 VOFA+ 波形输出，见"七、VOFA+ 波形调参" |

任何命令前面加 `@` 就变成**静默执行**——照常跑，但一个字节都不回显。是给 VOFA+ 的
滑块控件用的，见"七、VOFA+ 波形调参"里的"在 VOFA+ 里拖滑块改增益"。
...
| 5 | `spd_ref` | 目标速度 | 速度环 |
| 6 | `pwm_l` | 左轮 PWM = MOTO1(Limit 后)；`motor_enable=0` 时为 0 | 电机输出 |
| 7 | `pwm_r` | 右轮 PWM = MOTO2(Limit 后)；`motor_enable=0` 时为 0 | 电机输出 |

速度环的输出没有单列：它本来就是叠加在直立环给定上的，即 `ch1 - Med_Angle`。
转向差体现在 `pwm_l` / `pwm_r` 的差值上（`MOTO1 = vert - turn`，`MOTO2 = vert + turn`）。
...
4. VOFA+ 的命令输入框可以直接发 CLI 命令（记得末尾带 `\n`），所以调参不用再开一个终端。
   要绑滑块连续改增益，用下面"在 VOFA+ 里拖滑块改增益"的 `@` 前缀，否则回显会打乱波形。
...
命令前面加一个 `@`，就照常执行但**全程不往串口回一个字节**：

```
@pid vkp 520      改增益，没有任何回显和回执
@stop             急停，同样安静
```

为什么需要它：普通命令每条都要回"字符回显 + 执行结果 + `car> ` 提示符"共几十字节
ASCII。偶尔手敲一条无所谓，但拖一次滑块要连续下发几十条，这些 ASCII 会不断插进
JustFloat 的二进制流中间，VOFA+ 每次都得靠帧尾重新对齐，波形就没法看了。加 `@`
之后一个字节都不回，波形完全不受影响。

语法就是普通命令加个前缀，所以范围校验、改速度环增益时自动清积分这些**全部照旧生效**。

绑控件的步骤：

1. VOFA+ 里新建一个滑动条/输入框控件，绑到"发送"。
2. 发送模板填 `@pid vkp %.2f\n`，把 `vkp` 换成你要调的那一项。
3. 滑块范围别超过 `COMM_PID_GAIN_ABS_MAX`（2000），`med` 那一项是 ±20。
4. **结尾的 `\n` 不能少**，见下面的坑。

> **静默的代价是失败也没有提示。** 所以 `status` 里加了一行计数：
>
> ```
> quiet : ok=42 err=3
> ```
>
> 滑块拖了没反应时敲一次 `status`：两个数都不涨 = 命令根本没送到（查控件模板、
> 查串口是不是被别的软件占了）；`err` 在涨 = 命令到了但被拒（数值超范围、
> 命令名拼错，或者控件发的是 `5.2e2` 这种科学计数法——`cliStr2Float()` 只认
> `[+-]整数[.小数]`，那是当初为了躲开 `strtof` 的 8KB 才自己写的）。

> **控件模板漏了结尾的 `\n` 会怎样**：命令永远等不到行尾，`@` 的静默状态就一直
> 挂着，此后所有 CLI 输出都被丢弃，终端看上去和死机一模一样。这个坑太难查，所以
> `Cli_RxByte()` 做了兜底：静默行填满 64 字节缓冲还没等到换行时，就地丢弃并解除
> 静默，同时 `err` 加一。所以真漏了 `\n`，症状是 `err` 在涨而不是终端失声。

> 别一边在终端手敲命令一边拖滑块：两者共用同一个行缓冲。半截命令还在缓冲里时，
> 控件发来的 `@` 不在行首，不会被识别成静默前缀，会被当普通字符拼进你那行。

### 用滑块设速度目标（`spd`）

调速度环需要能给一个**看得见的阶跃**：把目标从 0 推到 20，看 ch4(`enc_sum`) 用多久
追上 ch5(`spd_ref`)、超调多少。`spd` 就是干这个的。

**开机默认就是 manual（目标 0）**，不用先“进入手动模式”。`status` 里应看到
`spdref: manual`。只有显式 `spd off` 才交还给蓝牙遥控。

```
spd 20      目标设成 20，ch5 立刻跳上去
spd 0       阶跃回 0，看它怎么收回来
spd off     交还给蓝牙遥控（此后 Fore/Back 才生效）
spd         不带参数，打印当前状态
```

绑滑块：新建控件，模板填 `@spd %.0f\n`，范围设 `-30 ~ 30`（`SPEED_Y` 的限幅，
超出会被夹住，`spd` 的回显打的是夹完的值，所以夹没夹一眼能看出来）。

> **为什么要单开一条通道，而不是让命令直接写 `Target_Speed`。**
> `Control()` 每一拍都会按蓝牙遥控的按键状态重算 `Target_Speed`——没收到遥控指令
> 就归零，收到了就按斜坡累加。所以外部写进去的值撑不过一个控制周期（10ms）就没了。
> `spd` 写的是 `pid.c` 里一份独立的手动目标，`Control()` 在遥控分支**之前**优先取用。
> 这也是为什么改这个功能时不能图省事直接 `extern int Target_Speed` 然后赋值：
> 那样命令会"执行成功"但毫无效果，比报错还难查。

生效期间的行为，都是安全考虑：

- **转向被强制为 0**。调速度环时不想让转向环掺进来。
- **10s 看门狗**（`MANUAL_SPEED_TIMEOUT_MS`）。超过这么久没有新的设定就把目标清 0，
  并清一次速度环积分，但**仍留在 manual**（不会被踢回 `remote`）。设完忘了车不会
  一直跑。拖滑块期间每次改动都会刷新它，所以只有撒手不管才会触发。
- **`start` 和 `stop` 只清目标为 0，不退出 manual**。否则上次调参剩下的 20 会在你
  `start` 的一瞬间把车推出去；同时也不再把模式打回 `remote`，VOFA 滑块可继续用。

> 看门狗到期清目标时会清速度环积分。理由和改 `skp`/`ski` 时一样：阶跃期间攒下的
> 积分对应的是那个目标值，目标归零后留着会让输出突跳。`spd off` 交还遥控时同样清零。
...
3. **速度环**：`pid skp` 从小往大加，`ski` 一般取 `skp/200` 量级起步。
   用 `spd 20` / `spd 0` 反复给阶跃，看 ch4(`enc_sum`) 追 ch5(`spd_ref`) 的
   上升时间和超调。速度环太强会把直立环带得前后晃 —— 表现是
   ch1(`roll_ref`) 大幅摆动。要用蓝牙遥控时再 `spd off`。
4. **转向环**：`pid tkp`，看 ch6/ch7（`pwm_l`/`pwm_r`）差值是否随转向变化。
5. 随时 `stop` 切电机。
